...
class Unit2 {
	...
	public Unit2(int type, Vector p, Vector v) {
		this.type = type;
		this.p = p;
		this.v = v;
	}
...
public class SmoothLife2 implements Drawing {
...
//	@GadgetDoubleLogarithmic(min = 1e-6, max = 1)
	...
	@GadgetLongRandomStream(seed = 1360985409318l)
	Long seed = 0l;
	Long seedLast = null;
	...
	void initTypes() {
		...
		for (int itA_ = 0; itA_ < nTypes; itA_++) {
			for (int itB_ = 0; itB_ < nTypes; itB_++) {
				...
				final FractalNoise1D fnP = new FractalNoise1D(3, Numeric.PHI, Numeric.PHI, rng.nextLong());
				final FractalNoise1D fnN = new FractalNoise1D(3, Numeric.PHI, Numeric.PHI, rng.nextLong());
				...
				interactions[itA][itB] = (Vector r) -> {
					...
					if (s < 0) {
						f = f.add(rn.mul(300*s));
					}
					if (d < forceRadius) {
						double x = d / forceRadius;
						double cp = fnP.getGaussian(x*forceRuggedness);
						double cn = fnN.getGaussian(x*forceRuggedness);

						Vector c = rn.mul(cp).add(rn.perp().mul(cn));
						f = f.add(c.mul(10 * mass[itB] * (1 - x*x*x*x)));
					}
	...
	public void draw(View view) {
		...
		for (Unit2 unit : units) {
			view.setFill(Color.hsb(360.0 * Numeric.PHI * unit.type, 0.6, 1.0));
